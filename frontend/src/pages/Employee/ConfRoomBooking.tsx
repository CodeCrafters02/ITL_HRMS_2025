...
const ConfRoomBooking = () => {
    ...
    const getRoomColor = (el: any) => {
        // Find if this room has any bookings during the WHOLE day
        const dayBookings = bookings.filter(b => b.room_details.layout_element_id === el.id && b.status !== 'rejected');
        
        // Simple visualization: 
        // Green if NO bookings today
        // Yellow if has some bookings
        // Red if completely booked (optional complex check)
        if (dayBookings.length === 0) return '#10B981';
        return '#FB923C';
    };
                    ...
                    {selectedRoom ? (
                        ...
                        <div className="flex flex-col items-center justify-center h-full opacity-50 text-center">
                            ...
                            <div className="mt-10 text-left w-full space-y-3 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
                                ...
                                <div className="flex items-center gap-2 text-xs font-bold"><span className="w-4 h-4 rounded bg-[#10B981]"></span> Fully Available Today</div>
                                <div className="flex items-center gap-2 text-xs font-bold"><span className="w-4 h-4 rounded bg-[#FB923C]"></span> Partly Booked Today</div>
                            </div>
                        </div>
                    )}
                </div>
