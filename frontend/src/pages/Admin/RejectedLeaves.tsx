...
const AdminRejectedLeaves = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Rejected Leaves'));
    }, [dispatch]);
    ...
    const fetchAll = async () => {
        setLoading(true);
        try {
            const url = new URL(`${API_BASE_URL}/app/rejected-leaves/`);
            ...
            if (!resp.ok) throw new Error(data?.detail || 'Failed to load rejected leaves');
    ...
    return (
        <div className="panel">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                <div>
                    <div className="text-xl font-bold">Rejected Leaves</div>
                    <div className="text-sm text-white-dark">Rejected leave logs with search and pagination.</div>
                </div>
