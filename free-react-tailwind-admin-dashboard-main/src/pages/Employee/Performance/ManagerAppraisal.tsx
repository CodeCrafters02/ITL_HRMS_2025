...
const ManagerAppraisal = () => {
    ...
    const [reporteeTargets, setReporteeTargets] = useState<ReporteeTarget[]>([]);
    const [selectedTarget, setSelectedTarget] = useState<ReporteeTarget | null>(null);
    ...
    useEffect(() => {
        dispatch(setPageTitle('Manager Evaluation'));
        loadTargets();
    }, [dispatch]);
    ...
    const loadTargets = async () => {
        try {
            ...
            const targetsList: ReporteeTarget[] = asArray(data.targets).filter((t: any) => t.relation === 'manager');
            setReporteeTargets(targetsList);
            ...
            console.error(err);
            setError(err.response?.data?.detail || 'Failed to load manager evaluation targets.');
            setLoading(false);
        }
    };

    const handleSelectTarget = async (target: ReporteeTarget, cycleId: number | null) => {
        ...
        try {
            setLoading(true);
            // Fetch manager questions for active cycle
            const qRes = await axios.get(`${API_BASE}/employee/appraisal-questions/?cycle=${cycleId}&role_type=manager`, { headers: headers() });
            ...
            console.error(err);
            setError('Failed to load manager questions.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmitFeedback = async () => {
        ...
        if (!window.confirm(`Are you sure you want to submit manager evaluation for ${selectedTarget.name}? You cannot edit ratings afterwards.`)) {
            return;
        }
        ...
        try {
            ...
            setSuccessMsg(`Manager evaluation submitted successfully for ${selectedTarget.name}!`);
            
            // Reload targets to refresh status
            const res = await axios.get(`${API_BASE}/employee/appraisal-evaluations/my_feedback_targets/`, { headers: headers() });
            const targetsList: ReporteeTarget[] = asArray(res.data.targets).filter((t: any) => t.relation === 'manager');
            setReporteeTargets(targetsList);
            ...
            console.error(err);
            setError(err.response?.data?.detail || 'Failed to submit manager feedback.');
        } finally {
            setSaving(false);
        }
    };

    if (loading && reporteeTargets.length === 0) {
        return (
    ...
    if (error && reporteeTargets.length === 0) {
        return (
    ...
    if (!activeCycleId || reporteeTargets.length === 0) {
        return (
            <div className="space-y-5 py-2 animate__animated animate__fadeIn">
                <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl p-8 text-center max-w-lg mx-auto">
                    ...
                    <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-1">No Manager Evaluations Assigned</h3>
                    <p className="text-xs text-gray-405 leading-relaxed mb-6">
                        You have no reportee evaluations assigned to you in the current appraisal cycle.
                    </p>
        ...
        <div className="space-y-6 py-2 animate__animated animate__fadeIn">
            ...
            <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">Evaluate your direct reportees for the current review cycle.</p>
                </div>
                <Link to="/employee/performance" className="px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-855 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-xl text-xs font-bold transition">
                    ← Performance Hub
                </Link>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
                ...
                <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl p-4 space-y-3">
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-wider block">Assigned Reportees ({reporteeTargets.length})</span>
                    <div className="space-y-2">
                        {reporteeTargets.map(target => {
                            const isSelected = selectedTarget?.id === target.id;
                            const colors = ['bg-emerald-500', 'bg-teal-500', 'bg-indigo-500', 'bg-violet-500', 'bg-cyan-500'];
                ...
                <div className="lg:col-span-2 space-y-4">
                    {selectedTarget ? (
                        <>
                            ...
                            {questions.length === 0 ? (
                                <div className="bg-white dark:bg-gray-900 border border-dashed border-gray-200 dark:border-gray-700 rounded-3xl py-12 text-center">
                                    ...
                                    <h4 className="text-sm font-bold text-gray-500">No questions mapped for manager evaluation</h4>
                                    <p className="text-[10px] text-gray-400 mt-1">Ask HR to map questionnaire for manager evaluation reviews.</p>
                                </div>
                            ) : (
                                            ...
                                            <div key={q.id} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl p-5 shadow-sm space-y-4">
                                                ...
                                                <div className="space-y-2 pl-9 pt-2">
                                                    ...
                                                    <textarea rows={3} placeholder="Provide specific feedback or examples to support this rating..."
                                                        ...
                                                        className="w-full bg-gray-50 dark:bg-gray-855 border border-gray-200 dark:border-gray-855 px-3 py-2 rounded-xl text-xs font-semibold text-gray-850 dark:text-white focus:outline-none resize-none focus:ring-1 focus:ring-teal-500" />
                                                </div>
                                            </div>
                                        );
                                    })}
                        ...
                        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-3xl p-8 text-center">
                            ...
                            <h4 className="text-sm font-bold text-gray-600 dark:text-gray-400">Select a Reportee</h4>
                            <p className="text-[10px] text-gray-400 mt-1 max-w-xs mx-auto">Choose a reportee from the list on the left to fill in your manager evaluation review.</p>
                        </div>
                    )}
                </div>
            </div>
