    ...
    const handleBulkToggle = async (field: string, value: boolean) => {
        try {
            const result = await Swal.fire({
                title: `Are you sure?`,
                text: `This will ${value ? 'enable' : 'disable'} Geofencing for ALL office locations.`,
                icon: 'warning',
        ...
        <div className="space-y-6">
            <div className="panel">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-5 border-b border-[#ebedf2] dark:border-[#1b2e4b] pb-5">
                    <div>
                        <h5 className="font-semibold text-lg dark:text-white-light">Geofencing Control</h5>
                        <p className="text-white-dark text-sm mt-1">Manage physical location policies for all office-based employees.</p>
                    </div>
                    ...
                    {!loading && locations.length > 0 && (
                        <div className="flex flex-wrap items-center gap-4 p-3 bg-primary/5 rounded-lg border border-primary/10">
                            <span className="text-xs font-bold uppercase text-primary">Master Control:</span>
                            <div className="flex items-center gap-6">
                                <div className="flex items-center gap-2">
                                    <label className="text-xs font-semibold cursor-pointer" htmlFor="master-gps">GPS Geofencing (All)</label>
                                        ...
                                        {isGpsGlobalEnabled && (
                                            <div className="p-4 bg-gray-50 dark:bg-black/10 rounded-lg space-y-4 border-l-2 border-primary">
                                                <div className="flex items-center justify-between">
                                                    <span className="text-xs font-bold uppercase text-white-dark">Office Coordinates</span>
                                                    <button
                                                        type="button"
