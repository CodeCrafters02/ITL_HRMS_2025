...
export default function Department() {
  ...
  const updateDepartment = async () => {
    if (!editId || !editName.trim()) return toast.error("Enter department name");

    try {
      await axiosInstance.put(`app/departments/${editId}/`, { department_name: editName });
      toast.success("Department updated successfully", { position: "bottom-right" });
      closeEditModal();
      fetchDepartments();
    } catch {
      toast.error("Failed to update department", { position: "bottom-right" });
    }
  };
            ...
            <div className="max-w-full overflow-x-auto">
              <Table>
                <TableHeader className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-white/[0.05]">
                  <TableRow>
                    <TableCell isHeader className="px-6 py-4 font-semibold text-gray-700 text-left text-sm dark:text-gray-300">S.No</TableCell>
                    <TableCell isHeader className="px-6 py-4 font-semibold text-gray-700 text-left text-sm dark:text-gray-300">Department Name</TableCell>
                  ...
                  {departments.length === 0 ? (
                    ...
                    departments.map((dept, idx) => (
                      <TableRow key={dept.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                        ...
                        <TableCell className="px-6 py-5 text-left text-lg font-medium text-gray-900 dark:text-white">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg p-8 w-full max-w-md">
            ...
            <p className="mb-6 text-gray-700 dark:text-gray-300">
              Are you sure you want to delete the department <span className="font-semibold">{deleteName}</span>?
            </p>
            <div className="flex justify-end gap-3">
              <button
